...
const Comment = ({ newsId, articleComments }) => {
  ...
  const getCurrentUser = async () => {
    try {
      ...
      const { data } = await axios.get(`https://newsletter-backend-xi.vercel.app/currentUser`, {
  ...
  const handleDelete = async (CommentId) => {
    if (CommentId !== null) {
      try {
        const response = await axios.delete(`https://newsletter-backend-xi.vercel.app/commentDelete/${CommentId}?articleID=${newsId}`);
  ...
  const createComment = async () => {
    try {
      const response = await axios.post(`https://newsletter-backend-xi.vercel.app/commentCreate/${newsId}`, {
        comment: comment,
        email: user?.email,
      });
