        ...
        <>
          {sections?.map((article, idx) => (
            <article key={idx} className="basis-0 grow">
              <p
                onClick={() => router.push(`/section?n=${article.section}`)}
                className="text-[14px] font-[700] text-[#666] mb-[4px] first-letter:text-[14px] first-letter:uppercase cursor-pointer"
              >
                {article.section}
              </p>
              <h2
                onClick={() => router.push(`/newsarticle?id=${article?._id}`)}
                className="text-[#0f151e] font-[600] text-[16px] mb-[8px] cursor-pointer"
              >
                {article.title}
              </h2>
